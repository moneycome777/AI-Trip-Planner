...
const ChatAssistant: React.FC<Props> = ({ messages, onSendMessage, isUpdating }) => {
  ...
  if (!isOpen) {
      return (
        <div className="fixed bottom-6 right-6 z-[3000] flex flex-col items-end gap-2 pb-[safe-area-inset-bottom]">
  ...
  return (
    <>
        {/* Mobile Backdrop - Locks background interaction */}
        <div 
            className="fixed inset-0 bg-black/20 backdrop-blur-[1px] z-[2999] lg:hidden animate-fadeIn" 
            onClick={() => setIsOpen(false)}
        />
        
        <div className="fixed inset-x-0 bottom-0 lg:left-auto lg:right-6 lg:bottom-6 z-[3000] flex flex-col items-end">
            <div className="bg-white w-full lg:w-96 h-[80vh] lg:h-[500px] lg:max-h-[85vh] rounded-t-3xl lg:rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col animate-slideUp">
                {/* Header */}
                ...
                {/* Input Area - Fixed height at the bottom of the container */}
                <div className="p-3 bg-white border-t border-slate-100 pb-[calc(12px+env(safe-area-inset-bottom,0px))]">
